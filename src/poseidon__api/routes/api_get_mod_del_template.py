...
from src.poseidon__db.helpers.add_template import add_new_template, attach_current_content_to_template, add_new_template_content
from src.poseidon__db.helpers.delete_template_by_id import delete_template_by_id
from src.poseidon__db.helpers.update_template import update_template
from src.poseidon__db.model.template_content import TemplateType
...
def nfk_mk_template(api_resource: ApiResource, body: ApiCreateTemplate) -> TJsonResponse:
    ...
    with transaction_commit():
        template = add_new_template(
            ...
            user_created_id=UUID(POSEIDON__SYS_USER_ID),
            name=body.name,
        )
    ...
    with transaction_commit():
        attach_current_content_to_template(
            ...
            user_modified_id=UUID(POSEIDON__SYS_USER_ID)
        )
...
def nfk_mod_template(api_resource: ApiResource, template_id: UUID, body: ApiCreateTemplate) -> TJsonResponse:
    with transaction_commit():
        template, template_content = update_template(
            ...
            user_id=UUID(POSEIDON__SYS_USER_ID),
            name=body.name,
            content=body.content,
            type=body.type,
            input_data_json_schema=body.input_data_json_schema,
